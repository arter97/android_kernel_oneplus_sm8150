...
static int synaptics_tddi_open_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    /* 1Wide refcap hi/ lo and feedback, Write 0x0F to F54_ANALOG_CTRL91 */
    buffer[0] = 0x0f;
    buffer[1] = 0x0f;
    buffer[2] = 0x0f;
    ...
    /* 2Increase RST_DUR to 1.53us, Write 0x5c to F54_ANALOG_CTRL99 */
    ...
    /* 3Write 0x02 to F54_ANALOG_CTRL182 (00)/00 and (00)/02 */
    ...
    /* 4Change the INT_DUR as ELEC_OPEN_INT_DUR_ONE */
    ...
    /* 5Capture raw capacitance (rt92) image 1 */
    /* Run Report Type 92 */
    ...
    /* 6Change the INT_DUR into ELEC_OPEN_INT_DUR_TWO */
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_CTRL99, 3, buffer);
    ...
    /* 7Capture raw capacitance (rt92) image 2 */
    /* Run Report Type 92 */
    ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_DATA_BASE, 92);//select report type 92
    if (ret < 0) {
    ...
    /* 8generate the delta image, which is equeal to image2 - image1 */
    ...
    /* 9restore the original configuration */
    ...
    /* 11phase 2, data calculation and verification */
...
static int synaptics_tddi_short_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    /* 4use the upper half as part 1 image */
    ...
    /* 5use the lower half as part 2 image */
