...
static int sec_enable_earsense_mode(struct chip_data_s6sy761 *chip_info,
				    bool enable)
{
	int ret = -1;
				...
				int max_num)
{
	...
	event_buff =
	    kzalloc(MAX_EVENT_COUNT * SEC_EVENT_BUFF_SIZE * (sizeof(uint8_t)),
		    GFP_KERNEL);
	if (!event_buff) {
...
int sec_fix_tmode(struct chip_data_s6sy761 *chip_info, u8 mode, u8 state)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client, SEC_CMD_STATEMANAGE_ON,
				 STATE_MANAGE_OFF);
	sec_mdelay(20);
	ret |=
	    touch_i2c_write_block(chip_info->client, SEC_CMD_CHG_SYSMODE,
				  sizeof(tBuff), tBuff);
	sec_mdelay(20);
...
static int sec_read_self(struct chip_data_s6sy761 *chip_info, u8 type,
			 char *data, int len)
{
	int ret = 0;
	...
	ret =
	    touch_i2c_write_byte(chip_info->client, SEC_CMD_SELF_RAW_TYPE,
				 type);
	if (ret < 0) {
	...
	sec_mdelay(50);
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_READ_TOUCH_SELF_RAWDATA,
				 data_len, data);
	if (ret < 0) {
...
static int sec_read_mutual(struct chip_data_s6sy761 *chip_info, u8 type,
			   char *data, int len)
{
	int ret = 0;
	u8 buf[2] = { 0 };
	...
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_READ_TOUCH_RAWDATA, len,
				 data);
	if (ret < 0) {
...
static void sec_self_delta_read(struct seq_file *s, void *chip_data)
{
	...
	for (x = 0; x < chip_info->hw_res->RX_NUM; x++) {
		temp_delta =
		    (data[x * 2 + rx_offset] << 8) | data[x * 2 + 1 +
							  rx_offset];
		seq_printf(s, "%4d, ", temp_delta);
	}
	seq_printf(s, "\n");
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	state =
	    touch_i2c_read_byte(chip_info->client, SET_CMD_SET_CHARGER_MODE);
	...
	state =
	    touch_i2c_read_word(chip_info->client, SEC_CMD_WAKEUP_GESTURE_MODE);
				     ...
				     int read_len)
{
	...
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_READ,
				 read_len, rawdata);
	if (ret < 0) {
				   ...
				   int read_len)
{
	...
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_CMD_TOUCH_DELTA_READ,
				      ...
				      int read_len)
{
	...
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_CMD_TOUCH_SELFDATA_READ,
				 read_len, self_data);
	if (ret < 0) {
...
static int sec_execute_selftest(struct seq_file *s,
				struct chip_data_s6sy761 *chip_info,
...
int sec_execute_p2ptest(struct seq_file *s, struct chip_data_s6sy761 *chip_info,
			struct sec_testdata *sec_testdata)
{
	int rc;
				...
				uint8_t item_index)
{
	...
	for (i = 0; i < item_cnt; i++) {
		item_header =
		    (struct sec_test_item_header *)(fw->data +
						    p_item_offset[i]);
			  ...
			  struct sec_testdata *sec_testdata)
{
	...
	int i = 0, j = 0, item_cnt = 0, iArrayIndex = 0, iArrayIndex_2 =
	    0, ret = -1;
	struct chip_data_s6sy761 *chip_info =
	    (struct chip_data_s6sy761 *)chip_data;
	...
	const uint32_t readselfbytes =
	    (sec_testdata->TX_NUM + sec_testdata->RX_NUM) * 2;
	struct sec_test_item_header *item_header = NULL;
	int32_t *p_mutual_p = NULL, *p_mutual_n = NULL, *p_mutualGap_p =
	    NULL, *p_mutualGap_n = NULL;
	int32_t *p_tx_offset_p = NULL, *p_tx_offset_n = NULL, *p_rx_offset_p =
	    NULL, *p_rx_offset_n = NULL;
	...
	if (eint_count == 10) {
		...
		err_cnt++;
		seq_printf(s,
			   "Step 0:eint_status is low, TP EINT direct short\n");
		goto ERR_INT;
	...
	if (sec_testdata->test_item & (1 << TYPE_MUTUAL_RAW_OFFSET_DATA_SDC)) {
		...
		item_offset =
		    search_for_item(sec_testdata->fw, item_cnt,
		...
		for (j = 0; j < sec_testdata->RX_NUM; j++) {
			for (i = 0; i < sec_testdata->TX_NUM; i++) {
				...
				if ((nodeData < p_mutual_n[iArrayIndex])
					...
					if (!err_cnt) {
						seq_printf(s,
							   "Step 1: mutual offset_data failed at data[%d][%d] = %d [%d,%d]\n",
							   i, j, nodeData,
							   p_mutual_n
							   [iArrayIndex],
							   p_mutual_p
		...
		for (j = 0; j < sec_testdata->RX_NUM; j++) {
			for (i = 0; i < sec_testdata->TX_NUM - 1; i++) {
				...
				if ((nodeGap > p_mutualGap_p[iArrayIndex])
					...
					if (!err_cnt) {
						seq_printf(s,
							   "Step 2-1:mutual node[%d, %d]=%d and node[%d, %d]=%d gap beyond [%d, %d]\n",
							   i, j, Buff16, i + 1,
							   j, Buff16_2,
							   p_mutualGap_n
							   [iArrayIndex],
		...
		for (j = 0; j < sec_testdata->RX_NUM - 1; j++) {
			for (i = 0; i < sec_testdata->TX_NUM; i++) {
				...
				if ((nodeGap > p_mutualGap_p[iArrayIndex])
					...
					if (!err_cnt) {
						seq_printf(s,
							   "Step 2-2:mutual node[%d, %d]=%d and node[%d, %d]=%d gap beyond [%d, %d]\n",
							   j, i, Buff16, j + 1,
							   i, Buff16_2,
							   p_mutualGap_n
							   [iArrayIndex],
	...
	if (sec_testdata->test_item & (1 << TYPE_SELF_RAW_OFFSET_DATA_SDC)) {
		...
		for (i = 0; i < sec_testdata->TX_NUM; i++) {
			...
			if ((nodeData < p_tx_offset_n[i])
				...
				if (!err_cnt) {
					seq_printf(s,
						   "Step 3-1:self_offset_tx_data failed at data[%d] = %d [%d,%d]\n",
						   i, nodeData,
						   p_tx_offset_n[i],
						   p_tx_offset_p[i]);
				}
		...
		for (i = 0; i < sec_testdata->RX_NUM; i++) {
			...
			if ((nodeData < p_rx_offset_n[i])
				...
				if (!err_cnt) {
					seq_printf(s,
						   "Step 3-2:self_offset_rx_data failed at data[%d] = %d [%d,%d]\n",
						   i, nodeData,
						   p_rx_offset_n[i],
						   p_rx_offset_p[i]);
				}
		...
		for (j = 0; j < sec_testdata->RX_NUM; j++) {
			for (i = 0; i < sec_testdata->TX_NUM; i++) {
				...
				if ((nodeData < p_p2p_n[iArrayIndex])) {
					...
					if (!err_cnt) {
						seq_printf(s,
							   "Step 4-1:p2p_min_data failed at data[%d][%d] = %d [%d]\n",
							   i, j, nodeData,
							   p_p2p_n
							   [iArrayIndex]);
					}
		...
		for (j = 0; j < sec_testdata->RX_NUM; j++) {
			for (i = 0; i < sec_testdata->TX_NUM; i++) {
				...
				if ((nodeData > p_p2p_p[iArrayIndex])) {
					...
					if (!err_cnt) {
						seq_printf(s,
							   "Step 4-2:p2p_max_data failed at data[%d][%d] = %d [%d]\n",
							   i, j, nodeData,
							   p_p2p_p
							   [iArrayIndex]);
					}
...
static void sec_ts_print_channel(struct chip_data_s6sy761 *chip_info)
{
	...
	for (i = 0;
	     i < (chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM) * 2;
	     i += 2) {
...
static void sec_ts_print_frame(struct chip_data_s6sy761 *chip_info, short *min,
			       short *max)
{
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
		...
		for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
			snprintf(pTmp, sizeof(pTmp), " %3d",
				 chip_info->
				 pFrame[(j * chip_info->hw_res->RX_NUM) + i]);

			if (chip_info->
			    pFrame[(j * chip_info->hw_res->RX_NUM) + i] < *min)
				*min =
				    chip_info->
				    pFrame[(j * chip_info->hw_res->RX_NUM) + i];

			if (chip_info->
			    pFrame[(j * chip_info->hw_res->RX_NUM) + i] > *max)
				*max =
				    chip_info->
				    pFrame[(j * chip_info->hw_res->RX_NUM) + i];
...
static int sec_ts_read_frame(struct chip_data_s6sy761 *chip_info, u8 type,
			     short *min, short *max, bool save_result)
{
	...
	/* read data */
	ret =
	    touch_i2c_read_block(chip_info->client, SEC_READ_TOUCH_RAWDATA,
				 readbytes, pRead);
	if (ret < 0) {
		TPD_DEBUG("%s: read rawdata failed!\n", __func__);
		goto ErrorRelease;
	...
	*min = *max = chip_info->pFrame[0];
	...
	for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
		for (j = 0; j < chip_info->hw_res->RX_NUM; j++)
			chip_info->pFrame[(j * chip_info->hw_res->TX_NUM) + i] =
			    temp[(i * chip_info->hw_res->RX_NUM) + j];
	}
...
void sec_ts_run_rawdata_all(void *chip_data, bool full_read)
{
	...
	for (i = 0; i < read_num; i++) {
		...
		ret =
		    sec_ts_read_frame(chip_info, test_type[i], &min, &max,
				      false);
		if (ret < 0) {
		...
		if (full_read) {
			ret =
			    sec_ts_read_channel(chip_info, test_type[i], &min,
						&max, false);
			if (ret < 0) {
