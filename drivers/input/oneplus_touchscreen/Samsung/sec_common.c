				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	do {
		...
		if (offset == 0)
			ret =
			    touch_i2c_read_block(ts->client, lv1cmd, length,
						 &read_lv1_buff[offset]);
		else
			ret =
			    touch_i2c_read(ts->client, NULL, 0,
...
void sec_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	for (m = 0; m < item_cnt; m++) {
		...
		p_data32 =
		    (int32_t *) (fw->data + p_item_offset[m] +
...
static int tp_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	snprintf(data_buf, 128,
		 ...
		 rtc_now_time.tm_mday, rtc_now_time.tm_hour,
...
static int calibrate_fops_read_func(struct seq_file *s, void *v)
{
	struct touchpanel_data *ts = s->private;
	struct sec_proc_operations *sec_ops =
	    (struct sec_proc_operations *)ts->private_data;
...
static int calibrate_data_read_func(struct seq_file *s, void *v)
{
	...
	if (!ts->touch_count) {
		...
		seq_printf(s,
			   "1 error, skip calibrate_data when touch on screen\n");
	}
	mutex_unlock(&ts->mutex);
	enable_irq(ts->irq);
		    ...
		    struct sec_proc_operations *sec_ops)
{
	...
	prEntry_tmp =
	    proc_create_data("calibration", 0666, ts->prEntry_tp,
	...
	prEntry_tmp =
	    proc_create_data("calibration_verify", 0666, ts->prEntry_tp,
			     &proc_verify_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
	...
	prEntry_tmp =
	    proc_create_data("calibrate_data", 0666, ts->prEntry_tp,
			     &proc_calibrate_data_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
			 __LINE__);
