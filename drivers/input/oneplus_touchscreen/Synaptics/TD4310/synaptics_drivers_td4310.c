...
void __attribute__ ((weak)) tfa_mutex_lock(bool enable)
{
	return;
}				//mutex avoid i2c transfer in tp reset process
				      ...
				      struct point_info *points, int max_num)
{
	...
	struct chip_data_td4310 *chip_info =
	    (struct chip_data_td4310 *)chip_data;
	char buf[8 * max_num];
...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	TPD_INFO("F55_SENSOR_QUERY_BASE  = 0x%x \n\
            ...
            F55_SENSOR_DATA_BASE    = 0x%x \n", reg_info->F55_SENSOR_QUERY_BASE, reg_info->F55_SENSOR_CMD_BASE, reg_info->F55_SENSOR_CTRL_BASE, reg_info->F55_SENSOR_DATA_BASE);
					 ...
					 struct panel_info *panel_data)
{
	...
	TPD_INFO
	    ("chip_info->reg_info.F01_RMI_DATA_BASE =  0x%x , bootloader_mode = 0x%x\n",
...
*/
static int synaptics_enable_interrupt(struct chip_data_td4310 *chip_info,
				      bool enable)
{
	int ret;
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F01_RMI_CTRL00 + 1,
				 abs_status_int);
	if (ret < 0) {
		TPD_INFO
				   ...
				   int is_suspended)
{
	...
	if ((device_status & 0xC0) == 0x80) {	/*unconfigured and not in flash mode */
		TPD_INFO
		    ("%s, IRQ_RESET ,interrupt_status = 0x%x, device_status = 0x%x\n",
...
static void synaptics_exit_esd_mode(void *chip_data)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F51_CUSTOM_CMD_BASE, 0x04);
	if (ret < 0) {
...
static int synaptics_configuration_init(struct chip_data_td4310 *chip_info,
					bool config)
{
	int ret = 0;
...
static int synaptics_glove_mode_enable(struct chip_data_td4310 *chip_info,
				       bool enable)
{
	int ret = 0;
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F12_2D_CTRL23);
	if (enable) {
		ret =
		    touch_i2c_write_byte(chip_info->client,
					 chip_info->reg_info.F12_2D_CTRL23,
...
static int synaptics_enable_black_gesture(struct chip_data_td4310 *chip_info,
					  bool enable)
{
	int ret;
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F12_2D_CTRL20, 3,
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F12_2D_CTRL27,
			     wakeup_gesture_enable_buf);

	return 0;
}

static int synaptics_enable_edge_limit(struct chip_data_td4310 *chip_info,
				       bool enable)
{
	int ret;
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_GRIP_CTRL);
				      ...
				      struct gesture_info *gesture)
{
	...
	regswipe =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_DATA + 0x18);

	TPD_DEBUG
	    ("gesture_buffer[0] = 0x%x, regswipe = 0x%x, gesture_buffer[1] = 0x%x, gesture_buffer[4] = 0x%x\n",
	...
	TPD_DETAIL("%s, gesture_sign = 0x%x, gesture_type = %d\n", __func__,
		   gesture_sign, gesture->gesture_type);

	if (gesture->gesture_type != UnkownGesture) {
		ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
		ret =
		    touch_i2c_read_block(chip_info->client,
					 chip_info->reg_info.F51_CUSTOM_DATA, 8,
					 &(coordinate_buf[0]));
		ret =
		    touch_i2c_read_block(chip_info->client,
					 chip_info->reg_info.F51_CUSTOM_DATA +
					 8, 8, &(coordinate_buf[8]));
		ret =
		    touch_i2c_read_block(chip_info->client,
					 chip_info->reg_info.F51_CUSTOM_DATA +
					 16, 8, &(coordinate_buf[16]));
		ret =
		    touch_i2c_read_block(chip_info->client,
					 chip_info->reg_info.F51_CUSTOM_DATA +
					 24, 1, &(coordinate_buf[24]));
		ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
		for (i = 0; i < 23; i += 2) {
			trspoint =
...
static int checkCMD_for_finger(struct chip_data_td4310 *chip_info)
{
	...
	if (chip_info->p_spuri_fp_touch == NULL) {
		TPD_INFO
		    ("checkCMD_for_finger() chip_info->p_spuri_fp_touch == NULL\n");
		return -1;
	}
	...
	do {
		...
		ret =
		    touch_i2c_read_byte(chip_info->client,
					...
					F54_ANALOG_COMMAND_BASE);
		flag_err++;
		if (chip_info->p_spuri_fp_touch->lcd_trigger_fp_check
		    && !chip_info->p_spuri_fp_touch->lcd_resume_ok) {
			TPD_INFO
			    ("checkCMD_for_finger() lcd_resume_ok=%d ret=%d flag_err=%d line=%d\n",
			     chip_info->p_spuri_fp_touch->lcd_resume_ok, ret,
			     flag_err, __LINE__);
			return -1;
		}
...
static int tddi_ratio_calculation(signed short *p_image,
				  struct f55_control_43 *p_ctrl_43, int tx,
				  int rx)
{
	int retval = 0;
	int i, j;
	...
	p_right_median =
	    (signed short *)kzalloc(rx_num * sizeof(short), GFP_KERNEL);
	if (!p_right_median) {
	...
	p_right_column_buf =
	    (signed short *)kzalloc(right_size * rx_num * sizeof(short),
				    GFP_KERNEL);
	...
	p_left_column_buf =
	    (signed short *)kzalloc(left_size * rx_num * sizeof(short),
				    GFP_KERNEL);
	...
	if (p_ctrl_43->swap_sensor_side) {
		...
		for (i = 0; i < rx_num; i++) {
			for (j = 0; j < left_size; j++) {
				p_left_column_buf[i * left_size + j] =
				    p_data_16[j * rx_num + i];
			}
		}
		// right side data
		...
		for (i = 0; i < rx_num; i++) {
			for (j = 0; j < right_size; j++) {
				p_right_column_buf[i * right_size + j] =
				    p_data_16[j * rx_num + i];
			}
		}
	} else {
	...
	for (i = 0; i < rx_num; i++) {
		p_left_median[i] =
		    find_median(p_left_column_buf + i * left_size, left_size);
		p_right_median[i] =
		    find_median(p_right_column_buf + i * right_size,
				right_size);
	}
...
static void synaptics_black_screen_RT136_test(void *chip_data, char *message)
{
	...
	if (ret < 0) {
		error_count++;
		TPD_INFO
		    ("[line:%d]read_baseline: touch_i2c_write_byte failed \n",
		     __LINE__);
	...
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, tx_num * rx_num * 2, raw_data);	//read data
	for (x = 0; x < 2; x++) {
		TPD_DEBUG_NTAG("[%d]: ", x);
		for (y = 0; y < rx_num; y++) {
			z = rx_num * x + y;
			...
			if (baseline_data < RT136_GESTURE_DOZE_LOW
			    || baseline_data > RT136_GESTURE_DOZE_HIGH) {
				TPD_INFO
				    ("Synaptic:Gesture doze test failed, [%d][%d] = %d[%d %d]\n",
				     x, y, baseline_data,
				     RT136_GESTURE_DOZE_LOW,
				     RT136_GESTURE_DOZE_HIGH);
				if (!error_count) {
					sprintf(buf,
						"Gesture Doze:[%d][%d] = %d[%d %d]\n",
						x, y, baseline_data,
						RT136_GESTURE_DOZE_LOW,
						RT136_GESTURE_DOZE_HIGH);
				}
...
static int synaptics_int_pin_test(struct seq_file *s, void *chip_data,
...
static int synaptics_tddi_RT136_test(struct seq_file *s, void *chip_data,
				     struct syna_testdata *syna_testdata)
{
	int ret = 0;
	uint8_t *raw_data = NULL;
	...
	if (ret > 0) {
		...
		seq_printf(s,
			   "write F54_ANALOG_DATA_BASE=0, F54_ANALOG_COMMAND_BASE=1,recovery tp touch\n");
		TPD_INFO
		    ("write F54_ANALOG_DATA_BASE=0, F54_ANALOG_COMMAND_BASE=1, recovery tp touch\n");
		return TEST_FAIL;
	}
	...
	raw_data =
	    kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 *
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (syna_testdata->fd >= 0) {
				store_to_file(syna_testdata->fd, "%d, ",
					      baseline_data);
			}

			TPD_DEBUG_NTAG("%d, ", baseline_data);

			if ((baseline_data < RT136_2D_LIMIT_LOW)
			    || (baseline_data > RT136_2D_LIMIT_HIGH)) {
				TPD_INFO
				    ("Synaptic:touchpanel failed, rt136[%d][%d] = %d\n",
				     x, y, baseline_data);
				if (!error_count) {
					seq_printf(s,
						   "touchpanel failed, rt136[%d][%d] = %d\n",
						   x, y, baseline_data);
				}
				error_count++;
			}
...
static int synaptics_tddi_RT3_doze_test(struct seq_file *s, void *chip_data,
					struct syna_testdata *syna_testdata)
{
	int ret = 0;
	uint8_t *raw_data = NULL;
	...
	if (ret > 0) {
		...
		seq_printf(s,
			   "checkCMD error, do not read RT92_doze test data\n");
		ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x00);	//select report type 0x00
		ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);	//get report 1
		seq_printf(s,
			   "write F54_ANALOG_DATA_BASE=0, F54_ANALOG_COMMAND_BASE=1,recovery tp touch\n");
	...
	for (x = 0; x < 2; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			TPD_DEBUG_NTAG("%d, ", baseline_data);
			if ((baseline_data < RT3_DOZE_LOW)
			    || (baseline_data > RT3_DOZE_HIGH)) {
				TPD_INFO
				    ("RT3 doze test error, TxRx[%d][%d] = %d[%d, %d]\n",
				     x, y, baseline_data, RT3_DOZE_LOW,
				     RT3_DOZE_HIGH);
				if (!error_count) {
					seq_printf(s,
						   "RT3 doze test error, TxRx[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   RT3_DOZE_LOW, RT3_DOZE_HIGH);
				}
				error_count++;
			}
		}
...
static int synaptics_tddi_open_test(struct seq_file *s, void *chip_data,
				    struct syna_testdata *syna_testdata)
{
	int ret = 0;
	int x, y, z;
	...
	TPD_INFO
	    ("TP Type %u, Duration [One:%d, Two:%d], Limit [One:%d, Two:%d]\n",
	     (uint8_t) tp_type, open_dur_one, open_dur_two, open_limit_one,
	     open_limit_two);
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F54_ANALOG_CTRL91,
				 sizeof(original_f54_ctrl91.data),
				 original_f54_ctrl91.data);
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Failed to read original data from f54_ctrl91\n");
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F54_ANALOG_CTRL99,
				 sizeof(original_f54_ctrl99.data),
				 original_f54_ctrl99.data);
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Failed to read original data from f54_ctrl99\n");
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F54_ANALOG_CTRL182,
				 sizeof(original_f54_ctrl182.data),
				 original_f54_ctrl182.data);
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Failed to read original data from f54_ctrl182\n");
	...
	ret =
	    touch_i2c_write_block(chip_info->client,
				  chip_info->reg_info.F54_ANALOG_CTRL91, 5,
				  buffer);
	if (ret < 0) {
		TPD_INFO("step 2 open test write F54_ANALOG_CTRL91 error\n");
		seq_printf(s,
			   "step 2 open test write F54_ANALOG_CTRL91 error\n");
	...
	ret =
	    touch_i2c_write_block(chip_info->client,
				  chip_info->reg_info.F54_ANALOG_CTRL99, 3,
				  buffer);
	if (ret < 0) {
		TPD_INFO("step 2 open test write F54_ANALOG_CTRL99 error\n");
		seq_printf(s,
			   "step 2 open test write F54_ANALOG_CTRL99 error\n");
	...
	ret =
	    touch_i2c_write_block(chip_info->client,
				  chip_info->reg_info.F54_ANALOG_CTRL182, 4,
				  buffer);
	if (ret < 0) {
		TPD_INFO("step 2 open test write F54_ANALOG_CTRL182 error\n");
		seq_printf(s,
			   "step 2 open test write F54_ANALOG_CTRL182 error\n");
	...
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test Change the INT_DUR as ELEC_OPEN_INT_DUR_ONE error\n");
		seq_printf(s,
			   "step 2 open Change the INT_DUR as ELEC_OPEN_INT_DUR_ONE error\n");
	...
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Capture raw capacitance (rt92) image 1 error\n");
		seq_printf(s,
			   "step 2 open test: Capture raw capacitance (rt92) image 1 error\n");
		store_to_file(syna_testdata->fd,
			      "Failed to capture raw rt92\n");
		return TEST_FAIL;
	}
	...
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Change the INT_DUR into ELEC_OPEN_INT_DUR_TWO  error\n");
		seq_printf(s,
			   "step 2 open test: Change the INT_DUR into ELEC_OPEN_INT_DUR_TWO  error\n");
	...
	if (ret < 0) {
		TPD_INFO
		    ("step 2 open test: Capture raw capacitance (rt92) image 2 error\n");
		seq_printf(s,
			   "step 2 open test: Capture raw capacitance (rt92) image 2 error\n");
		error_count++;
		goto RESTORE_CONFIG;
	...
	raw_data_delta_image =
	    kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM *
		    sizeof(signed short), GFP_KERNEL);
	...
	touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2, raw_data_2);	//read data
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			raw_data_delta_image[z] =
			    ((tmp_arg2 << 8) | tmp_arg1) - baseline_data;
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(syna_testdata->fd, "%d, ",
				      raw_data_delta_image[x *
							   syna_testdata->
							   RX_NUM + y]);
			if (raw_data_delta_image[x * syna_testdata->RX_NUM + y]
			    < open_limit_one) {
				TPD_INFO
				    ("step 2 open test: (%2d,%2d) = %4d < ELEC_OPEN_TEST_LIMIT_ONE(%d) fail\n",
				     x, y,
				     raw_data_delta_image[x *
							  syna_testdata->
							  RX_NUM + y],
	...
	synaptics_read_F54_base_reg(chip_info);
	tddi_ratio_calculation(raw_data_delta_image, &ctrl_43,
			       syna_testdata->TX_NUM, syna_testdata->RX_NUM);
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (raw_data_delta_image[x * syna_testdata->RX_NUM + y]
			    < open_limit_two) {
				TPD_INFO
				    ("step 2 open test: (%2d,%2d) = %4d < ELEC_OPEN_TEST_LIMIT_TWO(%d) fail\n",
				     x, y,
				     raw_data_delta_image[x *
							  syna_testdata->
							  RX_NUM + y],
...
static int synaptics_tddi_short_test(struct seq_file *s, void *chip_data,
				     struct syna_testdata *syna_testdata)
{
	int ret = 0;
	int x, y, z;
	...
	tddi_rt95_raw_data =
	    kzalloc(2 * tx_2d_num * syna_testdata->RX_NUM * 2 *
		    (sizeof(uint8_t)), GFP_KERNEL);
	...
	tddi_rt95_part_one =
	    kzalloc(tx_2d_num * syna_testdata->RX_NUM * sizeof(signed short),
		    GFP_KERNEL);
	...
	touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, 2 * tx_2d_num * syna_testdata->RX_NUM * 2, tddi_rt95_raw_data);	//read data

	/* 4、use the upper half as part 1 image */
	/* the data should be lower than TEST_LIMIT_PART1 ( fail, if > TEST_LIMIT_PART1 ) */
	TPD_DEBUG_NTAG
	    ("----------- step 3 short test : rt95 part 1 value ---------- \n ");
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(syna_testdata->fd, "%d, ",
				      tddi_rt95_part_one[x *
							 syna_testdata->RX_NUM +
							 y]);
			if (tddi_rt95_part_one[x * syna_testdata->RX_NUM + y] >
			    EXTEND_EE_SHORT_TEST_LIMIT_PART1) {
				TPD_INFO
				    ("step 3 short test : (%2d,%2d) = %4d > EXTEND_EE_SHORT_TEST_LIMIT_PART1(%d) fail\n",
				     x, y,
				     tddi_rt95_part_one[x *
							syna_testdata->RX_NUM +
							y],
	...
	TPD_DEBUG_NTAG
	    ("----------- step 3 short test : rt95 part 2 value ---------- \n ");
	for (x = 0; x < tx_2d_num; x++) {
		TPD_DEBUG_NTAG("[%2d] ", x);
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			z = syna_testdata->RX_NUM * x + y;
			tmp_arg1 =
			    tddi_rt95_raw_data[buffer_size + z * 2] & 0xff;
			tmp_arg2 =
			    tddi_rt95_raw_data[buffer_size + z * 2 + 1] & 0xff;
			tddi_rt95_part_two[syna_testdata->RX_NUM * x + y] =
			    ((tmp_arg2 << 8) | tmp_arg1);
	...
	synaptics_read_F54_base_reg(chip_info);
	tddi_ratio_calculation(tddi_rt95_part_two, &ctrl_43, tx_2d_num,
			       syna_testdata->RX_NUM);
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(syna_testdata->fd, "%d, ",
				      tddi_rt95_part_two[x *
							 syna_testdata->RX_NUM +
							 y]);
			if (tddi_rt95_part_two[x * syna_testdata->RX_NUM + y] <
			    EXTEND_EE_SHORT_TEST_LIMIT_PART2) {
				TPD_INFO
				    ("step 3 short test : (%2d,%2d) = %4d < EXTEND_EE_SHORT_TEST_LIMIT_PART2(%d) fail\n",
				     x, y,
				     tddi_rt95_part_two[x *
							syna_testdata->RX_NUM +
							y],
...
static void synaptics_baseline_blackscreen_read(struct seq_file *s,
						void *chip_data)
{
	int ret = 0;
	int x = 0, y = 0, z = 0;
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0);
...
static void synaptics_baseline_read(struct seq_file *s, void *chip_data)
{
	...
	for (i = 0; i < 2; i++) {
		if (i == 0) {
			...
			tx_num = 2;
			ret =
			    touch_i2c_write_byte(chip_info->client, 0xff, 0x01);
...
static void synaptics_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	ret =
	    touch_i2c_read_word(chip_info->client,
				chip_info->reg_info.F54_ANALOG_DATA06);
	seq_printf(s, "Interference Metric: 0x%x\n", ret);

	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F54_ANALOG_DATA10);
	seq_printf(s, "Current Noise State: 0x%x\n", ret);

	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F54_ANALOG_DATA17);
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F12_2D_DATA_BASE, 8, buf);
	if (ret < 0) {
...
static void synaptics_reserve_read(struct seq_file *s, void *chip_data)
{
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_CTRL36);
	TPD_INFO("TP ps status is %d\n", ret);
	seq_printf(s, "TP ps status is %d\n", ret);

	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_DATA57);
...
static int checkFlashState(struct chip_data_td4310 *chip_info)
{
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.SynaF34_FlashControl + 1);
...
static void re_scan_PDT(struct chip_data_td4310 *chip_info)
{
	...
	chip_info->reg_info.SynaF34Reflash_BlockNum =
	    chip_info->reg_info.F34_FLASH_DATA_BASE;
	chip_info->reg_info.SynaF34Reflash_BlockData =
	    chip_info->reg_info.F34_FLASH_DATA_BASE + 1;
	chip_info->reg_info.SynaF34ReflashQuery_BootID =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE;
	chip_info->reg_info.SynaF34ReflashQuery_FlashPropertyQuery =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE + 1;
	chip_info->reg_info.SynaF34ReflashQuery_FirmwareBlockSize =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE + 2;
	chip_info->reg_info.SynaF34ReflashQuery_FirmwareBlockCount =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE + 3;
	chip_info->reg_info.SynaF34ReflashQuery_ConfigBlockSize =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE + 3;
	chip_info->reg_info.SynaF34ReflashQuery_ConfigBlockCount =
	    chip_info->reg_info.F34_FLASH_QUERY_BASE + 3;
	...
	chip_info->reg_info.SynaF34_FlashControl =
	    chip_info->reg_info.F34_FLASH_DATA_BASE + 2;
}
					   ...
					   bool force)
{
	...
	if ((header.firmware_size + header.config_size + 0x100 +
	     header.disp_config_size) > (uint32_t) fw->size) {
		TPD_INFO
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F34_FLASH_CTRL_BASE, 4,
				 buf);
	CURRENT_FIRMWARE_ID =
	    (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	FIRMWARE_ID =
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.SynaF34_FlashControl, 0x0F);
	msleep(10);
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F01_RMI_CTRL_BASE);
	...
	re_scan_PDT(chip_info);
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.SynaF34ReflashQuery_BootID, 2,
			     buf);
	touch_i2c_write_block(chip_info->client,
			      chip_info->reg_info.SynaF34Reflash_BlockData, 2,
			      buf);
	...
	buf[1] = (0 & 0xff00) >> 8;
	touch_i2c_write_block(chip_info->client,
			      chip_info->reg_info.SynaF34Reflash_BlockNum, 2,
			      buf);
	...
	for (j = 0; j < firmware; j++) {
		...
		touch_i2c_write_block(chip_info->client,
				      chip_info->reg_info.
				      SynaF34Reflash_BlockData, block,
				      &Firmware_Data[j * block]);
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.SynaF34_FlashControl,
				     0x02);
		ret = checkFlashState(chip_info);
...
#ifdef UPDATE_DISPLAY_CONFIG
	TPD_INFO
	    ("update-----------------display_configuration ------------------update!\n");
	//a)write SynaF34Reflash_BlockNum to access
	buf[0] = 0;
	...
	for (j = 0; j < disp_configuration; j++) {
		...
		if (ret > 0) {
			TPD_INFO
			    ("Disp_Configuration:The status(Image) of flash data3 is %x, time = %d\n",
			     ret, j);
			return FW_UPDATE_ERROR;
		}
	}
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.SynaF34_FlashControl, 0x00);
	msleep(100);
...
static fp_touch_state synaptics_spurious_fp_check(void *chip_data)
{
	...
	if (TX_NUM * RX_NUM * (sizeof(int16_t)) > 1800) {
		TPD_INFO
		    ("%s,TX_NUM*RX_NUM*(sizeof(int16_t)>1800,There is not enough space\n",
		     __func__);
	...
	for (x = 1; x < TX_NUM - 2; x++) {
		...
		for (y = 1; y < SPURIOUS_FP_RX_NUM; y++) {
			...
			if (delta_data > SPURIOUS_FP_LIMIT) {
				if (!tp_debug)
					TPD_INFO
					    ("delta_data too large, delta_data = %d TX[%d] RX[%d]\n",
					     delta_data, x, y);
				err_count++;
			}
		}
		...
		if (err_count > 2) {
			...
			if (!tp_debug) {
				TPD_INFO
				    ("finger protect trigger!report_finger_protect = %d\n",
				     fp_touch_state);
				break;
			}
		}
...
static void synaptics_write_ps_status(void *chip_data, int ps_status)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F51_CUSTOM_CTRL36, tmp);
	if (ret < 0) {
		TPD_INFO("write ps_status error\n");
		return;
	}
	TPD_INFO
	    ("F51_CUSTOM_CTRL36=0x%x  write ps_status value 0x%x ok, ps %s \n",
	     chip_info->reg_info.F51_CUSTOM_CTRL36, tmp,
	     ps_status ? "near" : "far");
