...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	TPD_INFO("F55_SENSOR_QUERY_BASE  = 0x%x \n\
            ...
            edge function register: F51_CUSTOM_GRIP_CTRL = 0x%x ", reg_info->F55_SENSOR_QUERY_BASE, reg_info->F55_SENSOR_CMD_BASE, reg_info->F55_SENSOR_CTRL_BASE, reg_info->F55_SENSOR_DATA_BASE, reg_info->F51_CUSTOM_GRIP_CTRL);
 ...
 */
static int synaptics_enable_interrupt(struct chip_data_s4322 *chip_info,
				      bool enable)
{
	int ret;
				   ...
				   int is_suspended)
{
	...
	if (device_status) {
		TPD_INFO
		    ("%s, IRQ_EXCEPTION ,interrupt_status = 0x%x, device_status = 0x%x\n",
	...
	if (interrupt_status & 0x02) {
		...
		TPD_INFO
		    ("%s, tp fw auto reset detect, interrupt_status = 0x%x, device_status = 0x%x\n",
		     __func__, interrupt_status, device_status);
	}
	return result_event;
}
...
static void synaptics_exit_esd_mode(void *chip_data)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F51_CUSTOM_CMD_BASE, 0x2);
	if (ret < 0) {
...
static int synaptics_configuration_init(struct chip_data_s4322 *chip_info,
					bool config)
{
	int ret = 0;
...
static int synaptics_glove_mode_enable(struct chip_data_s4322 *chip_info,
				       bool enable)
{
	int ret = 0;
...
static int synaptics_enable_black_gesture(struct chip_data_s4322 *chip_info,
					  bool enable)
{
	int ret;
	...
	if (report_gesture_ctrl_buf[2] & 0x02) {
		TPD_INFO
		    ("Black gesture already open,can not write black gesture regiseter in suspend\n");
		return 0;
	}
	if (enable) {
...
static int synaptics_enable_edge_limit(struct chip_data_s4322 *chip_info,
				       bool enable)
{
	int ret;
...
static int synaptics_tddi_RT92_test(struct seq_file *s, void *chip_data,
				    struct syna_testdata *syna_testdata)
{
	int ret = 0;
	...
	struct test_header *ph =
	    (struct test_header *)(syna_testdata->fw->data);
	struct chip_data_s4322 *chip_info = (struct chip_data_s4322 *)chip_data;
	struct synaptics_register *reg_info = &chip_info->reg_info;
	...
	baseline_data_test =
	    (uint16_t *) (syna_testdata->fw->data + ph->array_limit_offset);
...
static int synaptics_tddi_RT92_doze_test(struct seq_file *s, void *chip_data,
					 struct syna_testdata *syna_testdata)
{
	int ret = 0;
	uint8_t *raw_data = NULL;
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (((x < 2) && (y < syna_testdata->RX_NUM))
			    || ((x == tx_2d_num) && (y == 0))) {
				TPD_DEBUG_NTAG("%d, ", baseline_data);
				if ((baseline_data < LIMIT_DOZE_LOW)
				    || (baseline_data > LIMIT_DOZE_HIGH)) {
					seq_printf(s,
						   "step 5:RT92 doze test error,"
						   "baseline_data[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   LIMIT_DOZE_LOW,
						   LIMIT_DOZE_HIGH);
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(syna_testdata->fd, "%d, ",
				      raw_data_delta_image[x *
							   ...
							   RX_NUM + y]);
			if (x < tx_2d_num
			    && raw_data_delta_image[x * syna_testdata->RX_NUM +
						    y] <
			    ELEC_OPEN_TEST_LIMIT_ONE) {
				TPD_INFO
				...
				if (!error_count) {
					seq_printf(s,
						   "step 2 open test Open One:(%2d,%2d) = %4d Limit:%d\n",
						   x, y,
						   raw_data_delta_image[x *
									syna_testdata->
									RX_NUM +
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (x < tx_2d_num
				...
				if (!error_count) {
					seq_printf(s,
						   "step 2 open test Open Two:(%2d,%2d)=%4d Limit:%d\n",
						   x, y,
						   raw_data_delta_image[x *
									syna_testdata->
									RX_NUM +
				     ...
				     struct syna_testdata *syna_testdata)
{
	...
	if (ret < 0) {
		TPD_INFO
		    ("step 3 short test Modify the Reset duration, Integration duration error\n");
		seq_printf(s,
			   "step 3 short test Modify the Reset duration, Integration duration error\n");
		error_count++;
		goto RESTORE_CONFIG;
	...
	if (ret < 0) {
		...
		seq_printf(s,
			   "step 3 short test Modify CBC Timing Control error\n");
		error_count++;
		goto RESTORE_CONFIG;
	}
	...
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, 2 * tx_2d_num * syna_testdata->RX_NUM * 2, tddi_rt95_raw_data);	//read data
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (tddi_rt95_part_one[x * syna_testdata->RX_NUM + y] >
				...
				if (!error_count) {
					seq_printf(s,
						   "step 3 short test Short One:(%2d,%2d)=%4d Limit:%d\n",
						   x, y,
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (tddi_rt95_part_two[x * syna_testdata->RX_NUM + y] <
				...
				if (!error_count) {
					seq_printf(s,
						   "step 3 short test Short Two:(%2d,%2d)=%4d Limit:%d\n",
						   x, y,
						   tddi_rt95_part_two[x *
								      syna_testdata->
								      RX_NUM +
				...
				struct syna_testdata *syna_testdata)
{
	...
	error_count +=
	    synaptics_tddi_RT92_doze_test(s, chip_data, syna_testdata);
...
static void synaptics_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F01_RMI_DATA_BASE - 1);
	...
	seq_printf(s,
		   "F51_CUSTOM_DATA95 Metal Detected Interrupt: 0x%x, bit1(in esd mode)\n",
		   ret);
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_GRIP_CTRL);
	seq_printf(s,
		   "edge control function register(0x%x) = 0x%x, bit0(Enable Grip Suppression)\n",
		   chip_info->reg_info.F51_CUSTOM_GRIP_CTRL, ret);

	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_CTRL48);
	seq_printf(s,
		   "ps status register(0x%x) = 0x%x, bit2(ps_status,1:near 0:far) bit1(Force enter ESD mode) bit0(ESD mode enable)\n\n",
...
static void synaptics_write_ps_status(void *chip_data, int ps_status)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F51_CUSTOM_CTRL48, tmp);
	if (ret < 0) {
		TPD_INFO("write ps_status error\n");
		return;
	}
	TPD_INFO
	    ("chip_info->reg_info.F51_CUSTOM_CTRL48=0x%x  write ps_status value 0x%x ok, ps %s \n",
	     chip_info->reg_info.F51_CUSTOM_CTRL48, tmp,
	     ps_status ? "near" : "far");
