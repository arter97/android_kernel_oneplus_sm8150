...
extern void input_report_key_reduce(struct input_dev *dev, unsigned int code,
				    int value);
/*------------------------------------------Fuction Declare----------------------------------------------*/
...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		msleep(150);
		ret =
		    synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL00,
						  0x80);
		if (ret < 0) {
...
static void judge_key_status_work_func(struct work_struct *work)
{
	...
	TPD_INFO
	    ("key_press_all_the_time = %d, detecte  key press all the time when touch lcd, force Cal touch key \n",
	     key_press_all_the_time);
...
static void int_key(struct synaptics_ts_data *ts)
{
	...
	case 3:		//force cal
		TPD_INFO
		    ("key code = %d, key press all the time, force Cal touch key\n",
		     button_key & 0x03);
...
static ssize_t synaptics_rmi4_baseline_show_s1302(struct device *dev, char *buf,
						  bool savefile)
{
	int ret = 0;
	int x, y;
	...
	if (!ts_g) {
		num_read_chars +=
		    sprintf(&(buf[num_read_chars]), "ts_g is null\n");
	...
	TPD_INFO
	    ("synaptics_rmi4_baseline_show_s1302  step 1:select report type 0x03\n");

	if (savefile) {
		getnstimeofday(&now_time);
		rtc_time_to_tm(now_time.tv_sec, &rtc_now_time);
		sprintf(data_buf,
			"/sdcard/tp_key_testlimit_%02d%02d%02d-%02d%02d%02d.csv",
			(rtc_now_time.tm_year + 1900) % 100,
			rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour + 8 >
			23 ? (rtc_now_time.tm_hour + 8 -
			      24) : rtc_now_time.tm_hour + 8,
	...
	if (ret < 0) {
		TPD_INFO
		    ("read_baseline: i2c_smbus_write_byte_data_s1302 failed \n");
		goto END;
	}
	...
	for (x = 0; x < 2; x++) {
		for (y = 0; y < 5; y++) {
			ret =
			    synaptics_rmi4_i2c_read_byte(ts_g->client,
			...
			if ((x == 0) && ((y == 3) || (y == 4))) {
				...
				if ((baseline_data <
					...
					*(baseline_data_test + count * 2 +
					...
					num_read_chars +=
					    sprintf(&(buf[num_read_chars]),
						    "conut = %d TPD error baseline_data[%d][%d] = %d[%d, %d]\n",
						    count, x, y, baseline_data,
						    *(baseline_data_test +
						      count * 2),
						    *(baseline_data_test +
						      count * 2 + 1));
	...
	ret =
	    synaptics_rmi4_i2c_write_word(ts_g->client, F54_ANALOG_COMMAND_BASE,
					  0x04);
	checkCMD();
 ...
 END:
	...
	//TPD_DEBUG("buf = %s\n", buf);
	ret =
	    synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_COMMAND_BASE,
					  0X02);
	msleep(60);
...
static ssize_t tp_reg_operate_read_func(struct file *file,
					...
					loff_t * ppos)
{
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(ts_g->client, 0xff,
					  tp_reg_address >> 8);
	if (ret < 0) {
		TPD_DEBUG("%s error %d\n", __func__, __LINE__);
		return -1;
	...
	ret =
	    synaptics_rmi4_i2c_read_byte(ts_g->client, tp_reg_address & 0x00ff);
...
static ssize_t tp_reg_operate_write_func(struct file *file,
					 const char __user * buffer,
					 size_t count, loff_t * ppos)
{
	int ret = 0;
...
static ssize_t tp_delta_show(struct file *file, char __user * buf, size_t size,
			     loff_t * ppos)
{
	int ret = 0;
	int x, y;
				...
				size_t size, loff_t * ppos)
{
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(ts_g->client,
	...
	for (x = 0; x < 2; x++) {
		...
		for (y = 0; y < 5; y++) {
			...
			num_read_chars +=
			    sprintf(kernel_buf + num_read_chars, "%5d",
				    baseline_data[x][y]);
		}
	}
...
static int synaptics_s1302_proc(void)
{
	...
	proc_entry =
	    proc_create_data("fw_update", 0444, procdir, &proc_firmware_update,
			     NULL);
	proc_entry =
	...
	proc_entry =
	    proc_create_data("tp_debug", 0666, procdir, &proc_debug, NULL);

	//guomingqiang@phone.bsp 2015-12-17 add for touch key debug node
	proc_entry =
	    proc_create_data("baseline_test", 0666, procdir,
			     &tp_baseline_test_proc_fops, NULL);
	proc_entry =
	    proc_create_data("tp_delta_data", 0644, procdir,
			     &tp_delta_data_proc_fops, NULL);
	proc_entry =
	    proc_create_data("tp_baseline_image", 0644, procdir,
			       ...
			       uint32_t data_len, bool force)
{
	...
	FIRMWARE_ID =
	    ...
	    Config_Data[3];
	TPD_INFO
	    ("CURRENT_FIRMWARE_ID is %x-----------, FIRMWARE_ID is %x-----------\n",
			      ...
			      const struct i2c_device_id *id)
{
	...
	register_device_proc("touch_key", touch_key_info.version,
			     touch_key_info.manufacture);
...
static void speedup_synaptics_resume(struct work_struct *work)
{
	...
	struct synaptics_ts_data *ts =
	    container_of(work, struct synaptics_ts_data, speed_up_work);
	TPD_INFO("%s \n", __func__);
	...
	if (bootloader_mode == 0x40) {
		TPD_INFO
		    (" %s Something terrible wrong, need update the Firmware again\n",
		     __func__);
#if 0				//only for test
		force_update = 1;
		ts_g->loading_fw = true;
		...
		TPD_INFO
		    ("%s firmware self-check bootloader_mode = 0x%x is ok\n",
