 ...
 */
static int synaptics_enable_interrupt(struct chip_data_s3508 *chip_info,
				      bool enable)
{
	int ret;
...
static int synaptics_configuration_init(struct chip_data_s3508 *chip_info,
					bool config)
{
	int ret = 0;
...
static int synaptics_glove_mode_enable(struct chip_data_s3508 *chip_info,
				       bool enable)
{
	int ret = 0;
...
static int synaptics_enable_black_gesture(struct chip_data_s3508 *chip_info,
					  bool enable)
{
	int ret;
...
static int synaptics_enable_edge_limit(struct chip_data_s3508 *chip_info,
				       bool enable)
{
	int ret;
				...
				struct syna_testdata *syna_testdata)
{
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (syna_testdata->fd >= 0) {
				...
				sys_write(syna_testdata->fd, data_buf,
					  strlen(data_buf));
			}

			if ((y < syna_testdata->RX_NUM - syna_testdata->key_RX)
			    && (x <
				syna_testdata->TX_NUM -
				syna_testdata->key_TX)) {
				...
				if ((baseline_data <
					...
					*(baseline_data_test + count * 2 +
					  1))) {
					TPD_INFO
					    ("Synaptic:touchpanel failed,enable_cbc=%d,raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
					     !enable_cbc, x, y, baseline_data,
					     *(baseline_data_test + count * 2),
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "touchpanel failed,enable_cbc=%d,raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   !enable_cbc, x, y,
						   baseline_data,
						   *(baseline_data_test +
						     count * 2),
			    ...
			    if ((x ==
				 syna_testdata->TX_NUM - syna_testdata->key_TX)
				&& (y >=
				    syna_testdata->RX_NUM -
				    syna_testdata->key_RX)) {
				TPD_DEBUG
				    ("touchkey test xy(%d,%d),baseline_data=%d,limite( %d, %d)\n ",
				     x, y, baseline_data,
				     *(baseline_data_test + count * 2),
				...
				if (((baseline_data) <
					...
					*(baseline_data_test + count * 2 +
					  1))) {
					TPD_INFO
					    ("Synaptic:touchkey failed,enable_cbc=%d,raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
					     !enable_cbc, x, y, baseline_data,
					     *(baseline_data_test + count * 2),
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "touchkey failed,enable_cbc=%d,raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   !enable_cbc, x, y,
						   baseline_data,
						   *(baseline_data_test +
						     count * 2),
	...
	do {
		...
		if (ret < 0) {
			TPD_INFO
			    ("read_baseline: touch_i2c_write_byte failed \n");
			goto END;
		}
		...
		if (enable_cbc) {
			...
			ret =
			    touch_i2c_write_byte(client,
						 reg_info->
						 F54_ANALOG_CONTROL_BASE + 23,
						 (tmp_arg1 | 0x10));
			ret =
			    touch_i2c_write_word(client,
						 reg_info->
			...
			tmp_arg2 = ret | 0x20;
			touch_i2c_write_byte(client,
					     reg_info->F54_ANALOG_CONTROL_BASE +
					     27, tmp_arg2);
			ret =
			    touch_i2c_write_word(client,
						 reg_info->
			...
			baseline_data_test =
			    (uint16_t *) (syna_testdata->fw->data +
					  ph->array_limitcbc_offset);
		} else {
			TPD_DEBUG("ret = %x, tmp_arg1 = %x, tmp_arg2 = %x\n",
	...
	ret =
	    touch_i2c_write_word(client, reg_info->F54_ANALOG_COMMAND_BASE,
				 0x04);
	checkCMD(chip_info, 30);
	...
	ret =
	    touch_i2c_write_word(client, reg_info->F54_ANALOG_DATA_BASE + 1,
				 0x0);
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_COMMAND_BASE, 0x01);	//get report
	//msleep(100);
	checkCMD(chip_info, 30);
	ret =
	    touch_i2c_read_block(client, reg_info->F54_ANALOG_DATA_BASE + 3, 7,
				 buffer);

	//guomingqiang@phone.bsp, 2016-06-27, add for tp test step2
	touch_i2c_write_byte(client, 0xff, 0x3);
	touch_i2c_read_block(client, reg_info->F55_SENSOR_CTRL01,
			     syna_testdata->RX_NUM, buffer_rx);
	touch_i2c_read_block(client, reg_info->F55_SENSOR_CTRL02,
		...
		if (buffer[x] == 0) {
			for (i = 0; i < 8; i++) {
				if ((buffer[x] & (1 << i)) != 0) {
					...
					for (j = 0; j < syna_testdata->RX_NUM;
					     j++) {
						if (sum_line_error ==
						    buffer_rx[j]) {
							error_count++;
							TPD_INFO
							    (" step 3 :check TRx-to-Ground, with rt25 error,  error_line is rx = %d\n",
							     buffer_rx[j]);
							seq_printf(s,
					...
					for (j = 0; j < syna_testdata->TX_NUM;
					     j++) {
						if (sum_line_error ==
						    buffer_tx[j]) {
							error_count++;
							TPD_INFO
							    (" step 3 :check TRx-to-Ground, with rt25, error_line is tx = %d\n",
							     buffer_tx[j]);
							seq_printf(s,
								   " step 3 :check TRx-to-Ground, with rt25, error_line is rx = %d\n",
								   buffer_rx
				...
				if ((buffer[x] & (1 << i)) != 0) {
					...
					for (j = 0; j < syna_testdata->RX_NUM;
						...
						if (sum_line_error ==
							...
							error_count++;
							TPD_INFO
							    (" step 4 :check tx-to-tx and tx-to-vdd error,  error_line is rx = %d\n",
							     buffer_rx[j]);
							seq_printf(s,
								   " step 4 :check tx-to-tx and tx-to-vdd error,  error_line is rx = %d\n",
								   buffer_rx
					...
					for (j = 0; j < syna_testdata->TX_NUM;
						...
						if (sum_line_error ==
							...
							error_count++;
							TPD_INFO
							    (" step 4 :check tx-to-tx and tx-to-vdd error, error_line is tx = %d\n",
							     buffer_tx[j]);
							seq_printf(s,
								   " step 4 :check tx-to-tx and tx-to-vdd error, error_line is rx = %d\n",
								   buffer_rx
	...
	for (y = 0; y < syna_testdata->RX_NUM; y++) {
		ret =
		    touch_i2c_read_byte(client,
					reg_info->F54_ANALOG_DATA_BASE + 3);
		tmp_arg1 = ret & 0xff;
		ret =
		    touch_i2c_read_byte(client,
					reg_info->F54_ANALOG_DATA_BASE + 3);
		...
		if (baseline_data > 100) {
			error_count++;
			TPD_INFO
			    (" step 5 :check the broken line, error_line is y =%d\n",
			     y);
			seq_printf(s,
				   "step 5 :check the broken line, error_line is y =%d, data[%d] = %d\n",
				   y, y, baseline_data);
			goto END;
		}
	}
	...
	for (y = 0;
	     ...
	     syna_testdata->key_TX - syna_testdata->key_RX; y++) {
		ret =
		    touch_i2c_read_byte(client,
					reg_info->F54_ANALOG_DATA_BASE + 3);
		tmp_arg1 = ret & 0xff;
		...
		if (unsigned_baseline_data < 10000) {
			error_count++;
			TPD_INFO
			    ("Step 7 : Check RT252 for random touch event, error_line is y =%d, data[%d] = %hu\n",
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM;
		...
		if (unsigned_baseline_data < 50 || unsigned_baseline_data > 975) {
			error_count++;
			TPD_INFO
			    ("Step 8 : Check RT254 for self raw data check, error_line is y =%d, data[%d] = %hu\n",
			     y, y, unsigned_baseline_data);
			seq_printf(s,
				   "Step 8 : Check RT254 for self raw data check, error_line is y =%d, data[%d] = %hu\n",
				   y, y, unsigned_baseline_data);
					   ...
					   bool force)
{
	...
	touch_i2c_write_block(chip_info->client,
			      ...
			      buf);
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.SynaF34_FlashControl, 0x03);
	msleep(2000);
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.SynaF34_FlashControl);
...
static void synaptics_data_logger_open(void *chip_data)
{
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_QUERY_BASE +
				0x05);
	...
	if (1 == ret) {
		...
		l_tmp =
		    touch_i2c_read_byte(chip_info->client,
					chip_info->reg_info.
					F51_CUSTOM_QUERY_BASE + 0x06);
		h_tmp =
		    touch_i2c_read_byte(chip_info->client,
					chip_info->reg_info.
					F51_CUSTOM_QUERY_BASE + 0x07);
		chip_info->d_log.loglength_addr = (h_tmp << 8) | l_tmp;
		chip_info->d_log.loginfo_addr =
		    chip_info->d_log.loglength_addr + 1;
		TPD_INFO
		    ("l_tmp = 0x%x h_tmp = 0x%x chip_info->d_log.loglength_addr = 0x%x chip_info->d_log.loginfo_addr = 0x%x\n",
		     l_tmp, h_tmp, chip_info->d_log.loglength_addr,
		     chip_info->d_log.loginfo_addr);
	}
