...
static int syna_get_report_data(struct syna_tcm_hcd *tcm_hcd,
				...
				unsigned int *data)
{
	...
	if (bits == 0 || bits > 32) {
		...
		secure_memcpy((unsigned char *)data, bits / 8,
			      &touch_report[byte_offset], bits / 8, bits / 8);
		return 0;
	}
...
static int syna_parse_report(struct syna_tcm_hcd *tcm_hcd)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_TIMESTAMP:
			...
			retval =
			    syna_get_report_data(tcm_hcd, offset, bits, &data);
			if (retval < 0) {
		...
		case TOUCH_OBJECT_N_INDEX:
			bits = config_data[idx++];
			retval =
			    syna_get_report_data(tcm_hcd, offset, bits, &obj);
			if (retval < 0) {
		...
		case TOUCH_REPORT_GESTURE_COORDINATE:
			bits = config_data[idx++];
			retval =
			    syna_get_report_data(tcm_hcd, offset, bits,
						 (unsigned int *)(&touch_data->
								  data_point
								  [0]));
			if (retval < 0) {
				TPD_INFO("Failed to get gesture double tap\n");
...
static int syna_set_default_report_config(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->default_config.buf) {
		retval = syna_tcm_alloc_mem(tcm_hcd, &tcm_hcd->config, length);
		if (retval < 0) {
			TPD_INFO("Failed to alloc mem\n");
			goto exit;
		}

		memcpy(tcm_hcd->config.buf, tcm_hcd->default_config.buf,
		       length);
...
static void syna_set_trigger_reason(struct syna_tcm_hcd *tcm_hcd,
...
static void syna_tcm_dispatch_response(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd,
				    &tcm_hcd->resp, tcm_hcd->payload_length);
	if (retval < 0) {
...
static int syna_tcm_continued_read(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_realloc_mem(tcm_hcd, &tcm_hcd->in, total_length);
	if (retval < 0) {
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = syna_tcm_read(tcm_hcd,
				       tcm_hcd->temp.buf, xfer_length + 2);
		if (retval < 0) {
				 ...
				 unsigned char *in_buf, unsigned int length)
{
 ...
 check_padding:
	...
	if (LEVEL_DEBUG == tp_debug) {
		syna_tcm_debug_message(&tcm_hcd->in.buf[4],
				       tcm_hcd->payload_length);
	}
 ...
 dispatch_reset:
	TPD_INFO
	    ("Firmware mode = 0x%02x, boot status 0x%02x, app status 0x%02x\n",
	     tcm_hcd->id_info.mode, tcm_hcd->boot_info.status,
	     tcm_hcd->app_status);
...
static u8 syna_trigger_reason(void *chip_data, int gesture_enable,
			      int is_suspended)
{
	struct syna_tcm_hcd *tcm_hcd = (struct syna_tcm_hcd *)chip_data;
...
static int syna_tcm_set_gesture_mode(struct syna_tcm_hcd *tcm_hcd, bool enable)
{
	...
	retval =
	    syna_tcm_get_dynamic_config(tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE,
					&config);
	if (retval < 0) {
	...
	if (enable) {
		if (!config) {
			...
			retval =
			    syna_tcm_set_dynamic_config(tcm_hcd,
							DC_IN_WAKEUP_GESTURE_MODE,
							true);
			if (retval < 0) {
				TPD_INFO
				    ("Failed to set dynamic gesture config\n");
				return retval;
			}
		}
	}
...
static int syna_tcm_normal_mode(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval =
	    syna_tcm_set_dynamic_config(tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE,
					false);
	if (retval < 0) {
...
static int syna_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_CHARGE:
		ret =
		    syna_tcm_set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED,
						flag ? 1 : 0);
		if (ret < 0) {
...
static int syna_hw_reset(struct syna_tcm_hcd *tcm_hcd,
...
static int syna_tcm_erase_flash(struct syna_tcm_hcd *tcm_hcd,
...
static int syna_tcm_write_flash(struct syna_tcm_hcd *tcm_hcd,
				...
				unsigned int datalen)
{
	...
	while (remaining_len) {
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, &out, xfer_len + 2);
		if (retval < 0) {
					  ...
					  const struct firmware *fw, bool force)
{
	...
	TPD_INFO("Write block size %d, page size %d, payload_size %d\n",
		 ...
		 reflash_hcd.page_size, reflash_hcd.max_write_payload_size);
	...
	ret =
	    syna_tcm_write_flash(tcm_hcd, &reflash_hcd, flash_addr, data, size);
	if (ret < 0) {
	...
	page_count =
	    ceil_div(image_info.disp_config.size, reflash_hcd.page_size);
...
static int syna_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if (gesture->gesture_type != UnkownGesture) {
		gesture->Point_start.x =
		    (touch_data->
		     data_point[0] | (touch_data->data_point[1] << 8));
		gesture->Point_start.y =
		    (touch_data->
		     data_point[2] | (touch_data->data_point[3] << 8));
		gesture->Point_end.x =
		    (touch_data->
		     data_point[4] | (touch_data->data_point[5] << 8));
		gesture->Point_end.y =
		    (touch_data->
		     data_point[6] | (touch_data->data_point[7] << 8));
		gesture->Point_1st.x =
		    (touch_data->
		     data_point[8] | (touch_data->data_point[9] << 8));
		gesture->Point_1st.y =
		    (touch_data->
		     data_point[10] | (touch_data->data_point[11] << 8));
		gesture->Point_2nd.x =
		    (touch_data->
		     data_point[12] | (touch_data->data_point[13] << 8));
		gesture->Point_2nd.y =
		    (touch_data->
		     data_point[14] | (touch_data->data_point[15] << 8));
		gesture->Point_3rd.x =
		    (touch_data->
		     data_point[16] | (touch_data->data_point[17] << 8));
		gesture->Point_3rd.y =
		    (touch_data->
		     data_point[18] | (touch_data->data_point[19] << 8));
		gesture->Point_4th.x =
		    (touch_data->
		     data_point[20] | (touch_data->data_point[21] << 8));
		gesture->Point_4th.y =
		    (touch_data->
		     data_point[22] | (touch_data->data_point[23] << 8));
	}
...
static int syna_tcm_get_frame_size_words(struct syna_tcm_hcd *tcm_hcd,
					 bool image_only)
{
	unsigned int rows;
	unsigned int cols;
				      ...
				      char *test_name, char *buf)
{
	...
	TPD_INFO("compare mode %d, test name %s, size %d\n", limit_block->mode,
		 limit_block->name, limit_block->size);
...
static int syna_testing_pt11(char *s, struct syna_tcm_hcd *tcm_hcd,
			     struct syna_testdata *syna_testdata)
{
	int retval;
	...
	retval = syna_tcm_write_message(tcm_hcd,
					...
					&resp_buf,
					&resp_buf_size,
					&resp_length, RESPONSE_TIMEOUT_MS_LONG);
	if (retval < 0) {
	...
	error_count =
	    syna_testing_limit_compare(s, syna_testdata, "PT11", resp_buf);
...
static int syna_testing_dynamic_range(char *s, struct syna_tcm_hcd *tcm_hcd,
				      struct syna_testdata *syna_testdata,
				      char *type)
{
	int retval;
	...
	error_count =
	    syna_testing_limit_compare(s, syna_testdata, type, resp_buf);
	if (error_count) {
...
static int syna_testing_noise(char *s, struct syna_tcm_hcd *tcm_hcd,
			      struct syna_testdata *syna_testdata, char *type)
{
	int retval;
	unsigned char out[2] = { 0 };
	unsigned char *resp_buf;
...
static int syna_testing_Doze_dynamic_range(char *s,
...
static int syna_testing_Doze_noise(char *s, struct syna_tcm_hcd *tcm_hcd,
				   struct syna_testdata *syna_testdata,
				   char *type)
{
	int retval;
...
static int syna_int_pin_test(char *s, void *chip_data,
			     struct syna_testdata *syna_testdata)
{
...
static void syna_tcm_black_screen_test(void *chip_data, char *message)
{
	...
	sprintf(data_buf,
		"/sdcard/tp_testlimit_gesture_%02d%02d%02d-%02d%02d%02d-utc.csv",
	...
	retval =
	    request_firmware(&fw, tcm_hcd->limit_name, &tcm_hcd->client->dev);
	if (retval < 0) {
	...
	error_count +=
	    syna_testing_noise(buffer, tcm_hcd, &syna_testdata, "GestureNoise");
	error_count +=
	    syna_testing_dynamic_range(buffer, tcm_hcd, &syna_testdata,
				       "GestureDynamic");
			   ...
			   struct syna_testdata *syna_testdata)
{
	...
	error_count +=
	    syna_testing_noise(buffer, tcm_hcd, syna_testdata, "Noise");
	error_count +=
	    syna_testing_dynamic_range(buffer, tcm_hcd, syna_testdata,
				       "Dynamic");
	...
	/*start Doze test */
	error_count +=
	    syna_testing_Doze_dynamic_range(buffer, tcm_hcd, syna_testdata,
					    "DozeDynamic");
	error_count +=
	    syna_testing_Doze_noise(buffer, tcm_hcd, syna_testdata,
				    "DozeNoise");
...
static int syna_tcm_collect_reports(struct syna_tcm_hcd *tcm_hcd,
...
static void syna_tcm_format_print(struct seq_file *s,
				  struct syna_tcm_hcd *tcm_hcd, char *buffer)
{
	unsigned int row, col;
	unsigned int rows, cols;
	short *pdata_16;
...
static int syna_tcm_init_device(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd,
				    &tcm_hcd->in, tcm_hcd->read_length + 2);
	...
	tcm_hcd->touch_hcd->touch_data.object_data =
	    (struct object_data *)kzalloc(sizeof(struct object_data) *
					  tcm_hcd->max_touch_num, GFP_KERNEL);
	...
	tcm_hcd->test_hcd =
	    (struct syna_tcm_test *)kzalloc(sizeof(struct syna_tcm_test),
					    GFP_KERNEL);
	if (!tcm_hcd->test_hcd) {
		retval = -ENOMEM;
