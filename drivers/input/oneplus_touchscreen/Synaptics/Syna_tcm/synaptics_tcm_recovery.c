...
static int recovery_add_data_entry(struct recovery_hcd *recovery_hcd,
				   unsigned char data)
{
...
static int recovery_add_padding(struct recovery_hcd *recovery_hcd,
				unsigned int *words)
{
	int retval;
...
static int recovery_parse_ihex(struct recovery_hcd *recovery_hcd)
{
	...
	for (record = 0; record < recovery_hcd->ihex_records; record++) {
		...
		retval = sscanf(&buf[record * IHEX_RECORD_SIZE],
				...
				&colon,
				&count, &addrh, &addrl, &type, &data0, &data1);
		if (retval != 7) {
		...
		if (type == 0x00) {
			if ((words % F35_CHUNK_SIZE_WORDS) == 0) {
				...
				retval =
				    recovery_add_data_entry(recovery_hcd, addr);
				if (retval < 0) {
					TPD_INFO("Failed to add data entry\n");
					return retval;
				}
				...
				retval =
				    recovery_add_data_entry(recovery_hcd,
							    addr >> 8);
				if (retval < 0) {
					TPD_INFO("Failed to add data entry\n");
					return retval;
...
static int recovery_get_fw_ihex(struct recovery_hcd *recovery_hcd, char *iHex)
{
	...
	retval =
	    request_firmware(&recovery_hcd->fw_entry, iHex,
			     &recovery_hcd->tcm_hcd->client->dev);
	if (retval < 0) {
	...
	secure_memcpy(recovery_hcd->ihex_buf, recovery_hcd->fw_entry->size,
