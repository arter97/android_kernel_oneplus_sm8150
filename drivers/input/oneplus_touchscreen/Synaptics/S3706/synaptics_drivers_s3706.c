...
static int synaptics_glove_mode_enable(struct chip_data_s3706 *chip_info,
				       bool enable)
{
	int ret = 0;
...
static int synaptics_capacity_test(struct seq_file *s,
				   ...
				   struct test_header *ph, uint8_t * raw_data,
				   uint8_t * data_buf)
{
	int ret = 0;
	int x = 0, y = 0, z = 0;
			...
			if ((y < syna_testdata->RX_NUM - syna_testdata->key_RX)
				...
				if ((baseline_data <
					...
					*(baseline_data_test + count * 2 +
					  1))) {
					TPD_INFO
					    ("Synaptic:touchpanel failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
					     x, y, baseline_data,
					     *(baseline_data_test + count * 2),
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "Step 1 :touchpanel failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   *(baseline_data_test +
						     count * 2),
						   *(baseline_data_test +
				...
				if (((baseline_data) <
					...
					*(baseline_data_test + count * 2 +
					  1))) {
					TPD_INFO
					    ("Synaptic:touchkey failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
					     x, y, baseline_data,
					     *(baseline_data_test + count * 2),
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "Step 1 :touchkey failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   *(baseline_data_test +
						     count * 2),
						   *(baseline_data_test +
	...
	TPD_DEBUG("ret = %x, tmp_arg1 = %x, tmp_arg2 = %x\n", ret, tmp_arg1,
		  (tmp_arg1 | 0x10));
	ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x14, 0x00);	/* Disable No SignalClarity */
	...
	baseline_data_test =
	    (uint16_t *) (syna_testdata->fw->data + ph->array_limitcbc_offset);
	/******write No Relax to 1******/
	ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);	/* force update */
	checkCMD(chip_info, 30);
					...
					*(baseline_data_test + count * 2 +
					...
					TPD_INFO
					     ...
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "Step 2 :touchpanel failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   *(baseline_data_test +
						     count * 2),
						   *(baseline_data_test +
					...
					*(baseline_data_test + count * 2 +
					...
					TPD_INFO
					     ...
					     *(baseline_data_test + count * 2 +
					       1));
					seq_printf(s,
						   "Step 2 :touchkey failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						   x, y, baseline_data,
						   *(baseline_data_test +
						     count * 2),
						   *(baseline_data_test +
				    ...
				    struct syna_testdata *syna_testdata)
{
	...
	buffer_rx =
	    kzalloc(syna_testdata->RX_NUM * (sizeof(uint8_t)), GFP_KERNEL);
	buffer_tx =
	    kzalloc(syna_testdata->TX_NUM * (sizeof(uint8_t)), GFP_KERNEL);
	...
	ret =
	    touch_i2c_write_word(chip_info->client,
				 chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
				 0x0);
	ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);	/*get report */
	/*msleep(100); */
	checkCMD(chip_info, 30);
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F55_SENSOR_CTRL01,
			     syna_testdata->RX_NUM, buffer_rx);
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F55_SENSOR_CTRL02,
...
static int synaptics_auto_test_rt26(struct seq_file *s,
				    struct chip_data_s3706 *chip_info,
				    struct syna_testdata *syna_testdata)
{
	int ret = 0;
	...
	for (x = 0; x < 7; x++) {
		...
		if (buffer[x] != 0) {
			if ((x == 0) && (buffer[x] & 0x01)) {	//pin - 0, these four pins should be test in RT100
				...
				error_count++;
				TPD_INFO
				    ("RT26 test fail!!!  data byte %d - data 0x%x\n",
				     x, buffer[x]);
				seq_printf(s,
					   " Step 4 :RT26 test fail!!!  data byte %d - data 0x%x\n",
					   x, buffer[x]);
			}
		}
	}
...
static int synaptics_auto_test_rt100(struct seq_file *s,
				     struct chip_data_s3706 *chip_info,
				     struct syna_testdata *syna_testdata,
				     uint8_t * raw_data)
{
	...
	touch_i2c_write_block(chip_info->client,
			      ...
			      0x1D, syna_testdata->RX_NUM, &temp_data[0]);
	ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);	/* force update */
	checkCMD(chip_info, 30);
	...
	for (x = 0, z = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			p_data_baseline1[x * syna_testdata->RX_NUM + y] =
			    baseline_data;
			z = z + 2;
			TPD_DEBUG_NTAG("%d, ",
				       p_data_baseline1[x *
	...
	for (i = 0; i < 4; i++) {
		...
		logical_pin =
		    GetLogicalPin(ExtendRT26_pin[i], syna_testdata->RX_NUM,
				  rx_physical);
		...
		ret =
		    touch_i2c_write_block(chip_info->client,
					  chip_info->reg_info.
					  F54_ANALOG_CONTROL_BASE + 0x1D,
					  syna_testdata->RX_NUM, &temp_data[0]);
		if (ret < 0) {
			error_count++;
			TPD_INFO
			    ("error: %s fail to set all F54 control_96 register after changing the cbc\n",
			     __func__);
			goto END;
		}
		...
		for (x = 0, z = 0; x < syna_testdata->TX_NUM; x++) {
			...
			for (y = 0; y < syna_testdata->RX_NUM; y++) {
				baseline_data =
				    raw_data[z] | (raw_data[z + 1] << 8);
				p_data_baseline2[x * syna_testdata->RX_NUM +
						 y] = baseline_data;
				z = z + 2;
				TPD_DEBUG_NTAG("%d, ",
					       p_data_baseline2[x *
		...
		for (x = 0; x < syna_testdata->TX_NUM; x++) {
			for (y = 0; y < syna_testdata->RX_NUM; y++) {
				p_data_delta[x * syna_testdata->RX_NUM + y] =
				    ABS(p_data_baseline1
					[x * syna_testdata->RX_NUM + y],
					p_data_baseline2[x *
							 syna_testdata->RX_NUM +
							 y]);
...
static int synaptics_auto_test_rt133(struct seq_file *s,
				     struct chip_data_s3706 *chip_info,
				     struct syna_testdata *syna_testdata)
{
	int ret = 0;
	...
	for (y = 0; y < syna_testdata->RX_NUM; y++) {
		...
		if (baseline_data > 100) {
			error_count++;
			TPD_INFO
			    (" step 6 :check the broken line, error_line is y =%d\n",
			     y);
			seq_printf(s,
...
static int synaptics_auto_test_rt150(struct seq_file *s,
				     struct chip_data_s3706 *chip_info,
				     struct syna_testdata *syna_testdata,
				     uint8_t * data_buf)
{
...
static int synaptics_auto_test_rt154(struct seq_file *s,
				     struct chip_data_s3706 *chip_info,
				     struct syna_testdata *syna_testdata,
				     uint8_t * data_buf)
{
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM;
		...
		if ((syna_testdata->key_RX != 0 || syna_testdata->key_TX != 0)
		    &&
		    (((y >
		       chip_info->hw_res->RX_NUM - syna_testdata->key_RX - 1)
...
static int synaptics_auto_test_rt155(struct seq_file *s,
				     struct chip_data_s3706 *chip_info,
				     struct syna_testdata *syna_testdata,
				     uint8_t * data_buf)
{
				...
				struct syna_testdata *syna_testdata)
{
	...
	if (eint_count == 10) {
		...
		error_count++;
		seq_printf(s,
			   "Step 0: eint_status is low, TP EINT direct stort\n");
		sprintf(data_buf,
	...
	synaptics_read_F54_base_reg(chip_info);
	ret =
	    synaptics_capacity_test(s, chip_info, syna_testdata, ph, raw_data,
				    data_buf);
	if (ret > 0) {
