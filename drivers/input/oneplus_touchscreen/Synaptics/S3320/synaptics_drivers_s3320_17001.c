				       ...
				       bool enable)
{
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F51_CUSTOM_CTRL50, ret);
				...
				struct syna_testdata *syna_testdata)
{
	...
	if (eint_count == 10) {
		...
		error_count++;
		snprintf(data_buf, sizeof(data_buf),
			 "step 0: begin to check INT-GND short item,"
	...
	if (ret < 0) {
		error_count++;
		snprintf(data_buf, sizeof(data_buf),
			 "step 1: Close CBC and test RT20 error,"
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if (syna_testdata->fd >= 0) {
				snprintf(data_buf, sizeof(data_buf), "%d, ",
					 baseline_data);
	...
	do {
		...
		if (ret < 0) {
			snprintf(data_buf, sizeof(data_buf),
				 "step 2:Open CBC and test RT3 error, "
				 "[line:%d]touch_i2c_write_byte failed \n",
				 __LINE__);
			TPD_INFO("%s", data_buf);
			error_count++;
	...
	for (x = 0; x < 4; x++) {
		if (buffer[x] != 0) {
			error_count++;
			snprintf(data_buf, sizeof(data_buf),
				 "step 3: check tx-to-tx and tx-to-vdd error,"
				 "x = %d, data = %d\n", x, buffer[x]);
			TPD_INFO("%s", data_buf);
			if (syna_testdata->fd >= 0) {
				sys_write(syna_testdata->fd, data_buf,
	...
	for (y = 0; y < syna_testdata->RX_NUM; y++) {
		...
		if (baseline_data > 100) {
			error_count++;
			snprintf(data_buf, sizeof(data_buf),
				 "step 4 :Check the broken line with RT133 error,"
	...
	touch_i2c_read_block(client, reg_info->F54_ANALOG_DATA_BASE + 3,
			     (syna_testdata->TX_NUM +
			      syna_testdata->RX_NUM) * 2, raw_data);
	if (syna_testdata->fd >= 0) {
		sys_write(syna_testdata->fd, "\nstep5: check doze mode\n", 1);
	}
	...
	for (x = 0; x < syna_testdata->RX_NUM; x++) {
		...
		if ((doze_data > LIMIT_DOZE) || (doze_data < LIMIT_DOZE_LOW)) {
			error_count++;
			snprintf(data_buf, sizeof(data_buf),
				 "step5: check abs doze error,"
				 "%d: %d not in( %d %d)\n", x, doze_data,
				 LIMIT_DOZE, LIMIT_DOZE_LOW);
			TPD_INFO("%s", data_buf);
			goto END;
		}
	}
...
static void synaptics_abs_doze_read(struct seq_file *s, void *chip_data)
{
	...
	raw_data =
	    kzalloc((chip_info->hw_res->TX_NUM +
		     chip_info->hw_res->RX_NUM) * 2 * (sizeof(uint8_t)),
		    GFP_KERNEL);
	if (!raw_data) {
		TPD_INFO("raw_data kzalloc error\n");
		return;
	...
	touch_i2c_read_block(chip_info->client,
			     ...
			     (chip_info->hw_res->TX_NUM +
			      chip_info->hw_res->RX_NUM) * 2, raw_data);
