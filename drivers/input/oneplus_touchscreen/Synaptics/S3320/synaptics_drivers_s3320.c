 ...
 */
static int synaptics_enable_interrupt(struct chip_data_s3320 *chip_info,
				      bool enable)
{
	int ret;
...
static int synaptics_reset_for_prepare(void *chip_data)
{
	...
	if (i2c_error_number == I2C_ERROR_MAX_TIME) {
		TPD_INFO
		    ("i2c_error_number=%d , pull down tp 3V ,then pull up\n",
		     i2c_error_number);
...
static int synaptics_configuration_init(struct chip_data_s3320 *chip_info,
					bool config)
{
	int ret = 0;
...
static int synaptics_glove_mode_enable(struct chip_data_s3320 *chip_info,
				       bool enable)
{
	int ret = 0;
...
static int synaptics_enable_black_gesture(struct chip_data_s3320 *chip_info,
					  bool enable)
{
	int ret;
...
static int synaptics_enable_edge_limit(struct chip_data_s3320 *chip_info,
				       bool enable)
{
	int ret;
	...
	TPD_INFO
	    ("edge function register F51_CUSTOM_CTRL50 = 0x%x, ?= 0x0427, enable=%d\n",
		...
		if (ts->spuri_fp_touch.lcd_trigger_fp_check
			...
			TPD_INFO
			    ("checkCMD_for_finger() lcd_resume_ok=%d ret=%d flag_err=%d line=%d\n",
			     ts->spuri_fp_touch.lcd_resume_ok, ret, flag_err,
			     __LINE__);
			return -1;
		}
				...
				struct syna_testdata *syna_testdata)
{
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			if ((y < (syna_testdata->RX_NUM - 1))
			    && (x < (syna_testdata->TX_NUM - 2))) {
			...
			} else if ((x > (syna_testdata->TX_NUM - 3))
				   && (y == (syna_testdata->RX_NUM - 1))) {
	...
	for (x = 0; x < 4; x++) {
		if (buffer[x] != 0) {
			...
			snprintf(data_buf, sizeof(data_buf),
				 "step 3: check tx-to-tx and tx-to-vdd error, x = %d, data = %d\n",
				 x, buffer[x]);
			TPD_INFO
			    ("step 3: check tx-to-tx and tx-to-vdd error, x = %d, data = %d\n",
			     x, buffer[x]);
	...
	for (y = 0; y < syna_testdata->RX_NUM; y++) {
		...
		if (baseline_data > 100) {
			error_count++;
			TPD_INFO
			    (" step 4 :check the broken line, error_line is y =%d\n",
			     y);
			seq_printf(s,
				   "step 4 :check the broken line, error_line is y =%d, data[%d] = %d\n",
				   y, y, baseline_data);
			goto END;
		}
	}
...
static void synaptics_self_delta_read(struct seq_file *s, void *chip_data)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F01_RMI_DATA_BASE, 0x42);
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F01_RMI_DATA_BASE, 0xE1);
...
static void synaptics_reserve_read(struct seq_file *s, void *chip_data)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F51_CUSTOM_CTRL42, 0x01);
	if (ret < 0) {
		TPD_INFO
		    ("[line:%d]read_baseline: touch_i2c_write_byte failed \n",
		     __LINE__);
