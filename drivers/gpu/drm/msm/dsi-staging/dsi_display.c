...
/* Read 60hz gamma para */
	...
	while ((check_sum_60hz !=
		(gamma_para_60hz[450] << 8) + gamma_para_60hz[451]) && (j < 5));
...
int dsi_display_set_aod_mode(struct drm_connector *connector, int level)
{
	...
	if (strcmp
		...
		(dsi_display->panel->name,
		 "samsung dsc cmd mode oneplus dsi panel") == 0) {
		printk(KERN_ERR
		       "oneplus SDC 2K OLED dsi_display_set_aod_mode\n");
	} else {
					...
					char *buf)
{
	...
	/* Last bit */
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "%02X ",
	...
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "%02X %02X ", msg->tx_len >> 8,
		     msg->tx_len & 0x00FF);
...
int dsi_display_get_dsi_on_command(struct drm_connector *connector, char *buf)
{
	...
	cmd =
	    &dsi_display->panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_ON];

	for (i = 0; i < cmd->count; i++) {
		count +=
		    dsi_display_get_mipi_dsi_msg(&cmd->cmds[i].msg,
						 &buf[count]);
	}
